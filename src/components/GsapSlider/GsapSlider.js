...
export default function GsapSlider() {
  ...
  {sidebarButtons.map((btn, i) => (
    <a
      key={i}
      href={btn.link}
      className="group flex items-center bg-blue-500 hover:bg-white transition rounded-lg shadow-md w-full overflow-hidden"
    ...
    >
      ...
      <div className="flex-1 text-left px-2 md:px-4 py-2">
        <span className="block text-white group-hover:text-black font-bold text-base md:text-sm leading-tight">
          {btn.label}
        </span>
      </div>
      <div className="flex items-center justify-center px-2 md:px-5">
        <span className="bg-white rounded-full w-8 md:w-10 h-8 md:h-10 flex items-center justify-center transition group-hover:bg-blue-500">
          <svg
